...
void TreeBuilder::initBlockages()
{
  ...
  if (libCell != nullptr) {
    ...
    // clang-format off
    debugPrint(logger_, CMS, "legalizer", 3, "buf width= {:0.3f} buf ht= {:0.3f} "
               "scalingUnit={}", bufferWidth_, bufferHeight_,
               techChar_->getLengthUnit());
    // clang-format on
  } else {
    logger_->error(
        CMS, 77, "No physical master cell found for cell {}.", buffer);
  }
                                   ...
                                   int scalingFactor)
{
  ...
  if (occupiedLocations_.find(bufferLoc) != occupiedLocations_.end()) {
    // clang-format off
    debugPrint(logger_, CMS, "legalizer", 4, "loc {} is already occupied",
	       bufferLoc);
    // clang-format on
    return false;
  }
  ...
  if (!coreArea.overlaps(loc)) {
    // clang-format off
    debugPrint(logger_, CMS, "legalizer", 4, "loc {} is outside core area",
	       bufferLoc);
    // clang-format on
    return false;
  }
  ...
  if (findBlockage(bufferLoc, scalingFactor, x1, y1, x2, y2)) {
    // clang-format off
    debugPrint(logger_, CMS, "legalizer", 4, "loc {} is in blockage ({:0.3f}"
...
bool TreeBuilder::isOccupiedLoc(const Point<double>& bufferLoc)
{
  ...
  debugPrint(logger_, CMS, "legalizer", 4, "loc {} is not occupied", bufferLoc);
  return false;
  // clang-format on
}
...
void TreeBuilder::commitLoc(const Point<double>& bufferLoc)
{
  ...
  debugPrint(logger_, CMS, "legalizer", 4, "loc {} has been committed, size={}",
...
void TreeBuilder::uncommitLoc(const Point<double>& bufferLoc)
{
  ...
  debugPrint(logger_, CMS, "legalizer", 4, "loc {} has been uncommitted, "
                                ...
                                const Point<double>& newLoc)
{
  ...
  debugPrint(logger_, CMS, "legalizer", 4, "move:{} -> {} has been committed, "
