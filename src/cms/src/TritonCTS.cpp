...
void TritonCTS::runTritonCts()
{
  ...
  populateTritonCTS();
  if (builders_.empty()) {
    logger_->warn(CMS, 82, "No valid clock nets in the design.");
  } else {
    ...
    for (int buf = 0; buf < wireSeg.getNumBuffers(); ++buf) {
      ...
      if (visitedMasters.count(master) == 0) {
        if (masterExists(master)) {
          ...
          logger_->error(CMS, 81, "Buffer {} is not in the loaded DB.", master);
        }
      }
    }
  });
                                 ...
                                 TreeBuilder* parent)
{
  ...
  if (driverNet->isSpecial()) {
    logger_->info(
        CMS, 116, "Special net \"{}\" skipped.", driverNet->getName());
  } else {
...
void TritonCTS::writeDataToDb()
{
  ...
  for (auto& builder : builders_) {
    ...
    if (sinks.size() < 2) {
      logger_->info(
          CMS, 124, "Clock net \"{}\"", builder->getClock().getName());
      logger_->info(CMS, 125, " Sinks {}", sinks.size());
    } else {
      countSinksPostDbWrite(builder.get(),
                            ...
                            sinks,
                            clkDummies);
      logger_->info(CMS, 98, "Clock net \"{}\"", builder->getClock().getName());
      logger_->info(CMS, 99, " Sinks {}", sinkCount);
      logger_->info(CMS, 100, " Leaf buffers {}", leafSinks);
      if (sinkCount > 0) {
        double avgWL = allSinkDistance / sinkCount;
        logger_->info(CMS, 101, " Average sink wire length {:.2f} um", avgWL);
      }
      logger_->info(CMS, 102, " Path depth {} - {}", minDepth, maxDepth);
      if (options_->dummyLoadEnabled()) {
        logger_->info(CMS, 207, " Leaf load cells {}", dummyLoadIndex_);
      }
    }
  }
}
  ...
  if (bufferList.empty()) {
    ...
    for (const std::string& buffer : bufferList) {
      if (db_->findMaster(buffer.c_str()) == nullptr) {
        logger_->error(
            CMS, 126, "No physical master cell found for buffer {}.", buffer);
      }
    }
  }
  ...
  while (lib_iter->hasNext()) {
    ...
    for (sta::LibertyCell* buffer : *lib->buffers()) {
      if (buffer->isClockCell() && isClockCellCandidate(buffer)) {
        ...
        // clang-format off
        debugPrint(logger_, CMS, "buffering", 1, "{} has clock cell attribute",
                   buffer->name());
        // clang-format on
      }
    }
...
void TritonCTS::setRootBuffer(const char* buffers)
{
  ...
  for (const std::string& buffer : bufferList) {
    if (db_->findMaster(buffer.c_str()) == nullptr) {
      logger_->error(
          CMS, 127, "No physical master cell found for buffer {}.", buffer);
    }
  }
...
std::string TritonCTS::selectSinkBuffer(std::vector<std::string>& buffers)
{
  ...
  // clang-format off
  debugPrint(logger_, CMS, "buffering", 1, "{} has been selected as sink "
    ...
    float totalCap)
{
  ...
  for (const std::string& name : buffers) {
    ...
    if (master == nullptr) {
      logger_->error(
          CMS, 117, "Physical master could not be found for cell '{}'", name);
    }
    sta::Cell* masterCell = network_->dbToSta(master);
    sta::LibertyCell* libCell = network_->libertyCell(masterCell);
    if (libCell == nullptr) {
      logger_->error(
          CMS, 112, "Liberty cell could not be found for cell '{}'", name);
    }
    ...
    // clang-format off
    debugPrint(logger_, CMS, "buffering", 1, "{} has cap limit:{}"
...
void TritonCTS::populateTritonCTS()
{
    ...
    for (auto clk : *sdc->clocks()) {
      ...
      for (auto net : clkNets) {
        if (allClkNets.find(net) != allClkNets.end()) {
          logger_->error(
              CMS, 114, "Clock {} overlaps a previous clock.", clkName);
        }
      }
  ...
  for (const auto& clockInfo : clockNetsInfo) {
    ...
    for (odb::dbNet* net : clockNets) {
      if (net != nullptr) {
        if (clkName.empty()) {
          logger_->info(CMS, 95, "Net \"{}\" found.", net->getName());
        } else {
          logger_->info(CMS,
                        7,
                        "Net \"{}\" found for clock \"{}\".",
  ...
  if (getNumClocks() == 0) {
    logger_->warn(CMS, 83, "No clock nets have been found.");
  }

  logger_->info(CMS, 8, "TritonCTS found {} clock nets.", getNumClocks());
                                   ...
                                   std::set<odb::dbNet*>& clkLeafNets)
{
  ...
  if (!rootSubNet) {
    logger_->error(
        CMS, 85, "Could not find the root of {}", clockNet.getName());
  }
  ...
  logger_->info(
      CMS, 12, "    Minimum number of buffers in the clock path: {}.", minPath);
  logger_->info(
      CMS, 13, "    Maximum number of buffers in the clock path: {}.", maxPath);

  if (numFixedNets_ > 0) {
    logger_->info(
        CMS, 14, "    {} clock nets were removed/fixed.", numFixedNets_);
  }

  logger_->info(CMS, 15, "    Created {} clock nets.", numClkNets_);
  ...
  logger_->info(
      CMS, 17, "    Max level of the clock tree: {}.", clockNet.getMaxLevel());
}
...
void TritonCTS::writeClockNDRsToDb(const std::set<odb::dbNet*>& clkLeafNets)
{
  ...
  for (int i = 1; i <= tech->getRoutingLayerCount(); i++) {
    ...
    // clang-format off
    debugPrint(logger_, CMS, "clustering", 1, "  NDR rule set to layer {} {} as "
...
void TritonCTS::createClockBuffers(Clock& clockNet, odb::dbModule* parent)
{
  ...
  logger_->info(CMS, 18, "    Created {} clock buffers.", numBuffers);
    ...
    subNet.forEachSink([&](ClockInst* inst) {
      if (inst->isClockBuffer()) {
        ...
        if (!sta::fuzzyEqual(cap, maxCap)) {
          needAdjust = true;
          // clang-format off
          debugPrint(logger_, CMS, "dummy load", 1, "{} => {} "
    ...
    const std::vector<sta::LibertyCell*>& dummyCandidates)
{
  ...
  // clang-format off
  debugPrint(logger_, CMS, "dummy load", 1, "insertDummyCell {} at {}",
  ...
  if (driver2subnet_.find(inst) == driver2subnet_.end()) {
    logger_->error(
        CMS, 120, "Subnet was not found for clock buffer {}.", inst->getName());
    return nullptr;
  }
                                     ...
                                     odb::dbInst*& dummyInst)
{
  ...
  // clang-format off
  debugPrint(logger_, CMS, "dummy load", 1, "  placed dummy instance {} at {}",
                                 ...
                                 ClockInst& dummyClock)
{
  ...
  if (sinkInst == nullptr) {
    logger_->error(
        CMS, 119, "Phyiscal instance {} is not found.", inst->getName());
  }
                                     ...
                                     odb::dbITerm* iterm)
{
  ...
  logger_->warn(CMS, 2, "No paths found for pin {}.", iterm->getName());
  return clkPathArrival;
}
                                ...
                                TreeBuilder* registerBuilder)
{
  ...
  if (numBuffers == 0) {
    // clang-format off
    debugPrint(logger_, CMS, "insertion delay", 1, "no delay buffers are needed"
               " to adjust latencies");
    // clang-format on
    return;
  }
  // clang-format off
  debugPrint(logger_, CMS, "insertion delay", 1, "{} delay buffers are needed"
