    ...
    const bool secondLevel)
{
  ...
  if (!secondLevel) {
    clock_.forEachSink([&](ClockInst& inst) {
      ...
      if (!fuzzyEqual(inst.getInsertionDelay(), 0.0, 1e-6)) {
        ...
        // clang-format off
	debugPrint(logger_, CMS, "clustering", 1, "sink {} has insDelay {} at {}",
  ...
  if (options_->isSinkClusteringSizeSet() && options_->isMaxDiameterSet()) {
    // clang-format off
      debugPrint(logger_, CMS, "clustering", 1, "**** match.run({}, {}, {}) ****",
  ...
  for (const std::vector<unsigned>& cluster :
    ...
    if (cluster.size() > 1) {
      ...
      for (auto point_idx : cluster) {
        ...
        if (mapLocationToSink_.find(mapPoint) == mapLocationToSink_.end()) {
          logger_->error(CMS, 79, "Sink not found.");
        }
        xSum += point.first;
        ySum += point.second;
	...
	debugPrint(logger_, CMS, "legalizer", 2,
...
void HTreeBuilder::initSinkRegion()
{
  ...
  // clang-format off
  debugPrint(logger_, CMS, "clustering", 1, "maxDiameter={:0.3f} = "
  ...
  if (topLevelSinks.size() <= min_clustering_sinks_
    ...
    logger_->info(CMS, 23, " Original sink region: {}.", sinkRegionDbu);
  ...
  logger_->info(CMS, 24, " Normalized sink region: {}.", sinkRegion_);
  logger_->info(CMS, 25, "    Width:  {:.4f}.", sinkRegion_.getWidth());
  logger_->info(CMS, 26, "    Height: {:.4f}.", sinkRegion_.getHeight());
}
                                       ...
                                       Point<double>& bestLoc)
{
  ...
  // clang-format off
  debugPrint(logger_, CMS, "legalizer", 3, "{} currDist={:0.3f} != "
    ...
    double& bestSinkDist)
{
  if (fuzzyEqual(computeDist(newLoc, parentPoint), targetDist)
    ...
    // clang-format off
    debugPrint(logger_, CMS, "legalizer", 3, "adjustBestLegalLoc: branchPt "
    ...
    odb::Direction2D direction)
{
  ...
  if (findBlockage(point, scalingFactor, x1, y1, x2, y2)) {
    ...
    double py = parentPoint.getY();
    // clang-format off
    debugPrint(logger_, CMS, "legalizer", 3, "  {} corner {} of Manhattan "
                                        ...
                                        double& bestSinkDist)
{
  if (fuzzyEqual(computeDist(newLoc, parentPoint), targetDist)
    ...
    // clang-format off
    debugPrint(logger_, CMS, "legalizer", 3, "adjustBeyondBlockage: branchPt "
	       "move:{}=>{} is legal, dist={:0.3f}, sinkDist={:0.3f}",
	       oldLoc, newLoc, targetDist, sinkDist);
  } else {
    debugPrint(logger_, CMS, "legalizer", 3, "adjustBeyondBlockage: branchPt "
  ...
  for (int levelIdx = 0; levelIdx < topologyForEachLevel_.size(); ++levelIdx) {
    ...
    for (unsigned idx = 0; idx < topology.getBranchingPointSize(); ++idx) {
      ...
      if (!isOccupiedLoc(branchPoint)
        ...
        // clang-format off
        debugPrint(logger_, CMS, "legalizer", 1,
...
void HTreeBuilder::legalize()
{
  ...
  // clang-format off
  debugPrint(logger_, CMS, "legalizer", 3, "legalize: top buf loc:{}->{}",
  ...
  for (int levelIdx = 0; levelIdx < topologyForEachLevel_.size(); ++levelIdx) {
	...
	debugPrint(logger_, CMS, "legalizer", 1,
        ...
        newLocation = adjustBeyondBlockage(branchPoint,
                                           ...
                                           branch_point_dir);
        // clang-format off
	debugPrint(logger_, CMS, "legalizer", 3,
...
{
  logger_->info(
      CMS, 27, "Generating H-Tree topology for net {}.", clock_.getName());
  logger_->info(CMS, 28, " Total number of sinks: {}.", clock_.getNumSinks());
  if (options_->getSinkClustering()) {
    if (options_->getSinkClusteringUseMaxCap()) {
      logger_->info(
          CMS, 90, " Sinks will be clustered based on buffer max cap.");
    } else {
      logger_->info(CMS,
                    29,
  ...
  logger_->info(
      CMS, 30, " Number of static layers: {}.", options_->getNumStaticLayers());
  ...
  createClockSubNets();
  // clang-format off
  debugPrint(logger_, CMS, "legalizer", 3, "Htree file {} has been generated",
             plotHTree());
  debugPrint(logger_, CMS, "legalizer", 3, "Run 'obsAwareCts.py cts.clk.buffer'"
                                        ...
                                        const double height)
{
  ...
  logger_->info(CMS, 34, "    Segment length (rounded): {}.", segmentLength);
...
void HTreeBuilder::createClockSubNets()
{
  ...
  // clang-format off
  if (center != legalCenter) {
    debugPrint(logger_, CMS, "legalizer", 2, "createClockSubNets: "
  ...
  for (int levelIdx = 1; levelIdx < topologyForEachLevel_.size(); ++levelIdx) {
    ...
    topology.forEachBranchingPoint([&](unsigned idx,
      ...
      // clang-format off
      if (branchPoint != legalBranchPoint) {
	debugPrint(logger_, CMS, "legalizer", 2, "createClockSubNets level {} "
		   "{} : {} => {}", levelIdx+1, "clkbuf_" +
      ...
      [&](unsigned idx, Point<double> branchPoint) {
        ...
        for (const Point<double>& loc : sinkLocs) {
          if (mapLocationToSink_.find(loc) == mapLocationToSink_.end()) {
            logger_->error(CMS, 80, "Sink not found.");
          }
  ...
  logger_->info(CMS, 35, " Number of sinks covered: {}.", numSinks);
}
...
void HTreeBuilder::createSingleBufferClockNet()
{
  ...
  // clang-format off
  if (center != legalCenter) {
    debugPrint(logger_, CMS, "legalizer", 2, "createSingleBufferClockNet "
...
void SegmentBuilder::build(const std::string& forceBuffer)
{
  ...
  for (unsigned techCharWireIdx : techCharWires_) {
	...
	debugPrint(getTree()->getLogger(), CMS, "legalizer", 2,
